...
function qoo_min(string $area, string $step, string $opt, string $col): ?int {
  // вернёт MIN(col) среди всех мастеров (staff_id>0), где значение не NULL
  try{
    $db = pdo();
    $st = $db->prepare("
      SELECT MIN($col) AS v
...
/* ===== «от …» для одной опции: минимумы по всем мастерам с fallback на config ===== */
...
function opt_min_points(string $areaKey, string $stepKey, array $node, string $optId): int {
  // 1€ = 1 балл, берём от минимальной цены
...
if (isset($_GET['go']) && $_GET['go'] === 'prev') {
  ...
  if ($from > 1) {
    header('Location: ' . quiz_url(['step'=>$from-1])); exit;
  }
...
function progress_text($lang,$MSG,$cur,$total){
  return sprintf(t('progress',$lang,$MSG), (int)$cur, (int)$total);
}
        ...
        <div class="grid">
          <?php
            ...
            foreach (($node['options'] ?? []) as $id=>$o):
              ...
              $hidePrice = opt_hide_price($node,$id);
              $textRaw   = $o['text'] ?? '';
          ?>
            <label class="option">
              ...
              <div class="chip">
                ...
                <?php if (!$hidePrice): ?>
                  <div class="price price--muted"><?= h(t('from',$lang,$MSG)) ?> <?= (int)$minEur ?> €</div>
                <?php endif; ?>
                <div class="price price--muted">⏱ <?= h(t('from',$lang,$MSG)) ?> <?= h(fmt_duration_hm((int)$minMin,$lang)) ?></div>
                <div class="price price--muted">🌸 <?= h(t('from',$lang,$MSG)) ?> +<?= (int)$minPts ?> <?=h(t('points',$lang,$MSG))?></div>
