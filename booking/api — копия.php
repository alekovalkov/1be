...
function table_exists(PDO $pdo, string $table): bool {
  // НЕЛЬЗЯ использовать плейсхолдер в SHOW, поэтому экранируем вручную
  $q = $pdo->query("SHOW TABLES LIKE " . $pdo->quote($table));
  if (!$q) return false;
  return (bool)$q->fetchColumn();
}
function col_exists(PDO $pdo, string $table, string $col): bool {
  $st = $pdo->query("SHOW COLUMNS FROM `$table` LIKE ".$pdo->quote($col)); return (bool)$st->fetch();
}
function pick_col(PDO $pdo, string $table, array $cands): ?string {
  foreach ($cands as $c) if (col_exists($pdo,$table,$c)) return $c; return null;
}
...
function pick_available_staff(PDO $db, string $date, string $time, int $duration, ?int $salonId): int {
  ...
  foreach($wh as $r){
    ...
    // time_off (с учётом разных имён колонок; защищаемся от пустого IN)
  $aStart = $startDT->format('Y-m-d H:i:s');
  $aEnd   = $endDT->format('Y-m-d H:i:s');

  $off = [];
  if (!$staffIds) {
    // уже есть проверка выше, но дублируем на всякий случай
    return 0;
  }
...
if ($action === 'meta') {
  ...
  try {
    ...
    $staff  = $db->query("SELECT id,name,is_active,COALESCE(tz,'".DEFAULT_TZ."') AS tz FROM staff WHERE is_active=1 ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
...
if (($_GET['action'] ?? $_POST['action'] ?? '') === 'book') {
  try {
    ...
    if ($staff_id <= 0) {
      $autoSid = pick_available_staff($db, $date, $time, (int)$duration, $salon_id > 0 ? $salon_id : null);
      if ($autoSid > 0) {
        $staff_id = $autoSid;
        if ($salon_id <= 0) {
...
// === Сколько баллов дать за визит ===
// правило: 1 € = 1 балл (дробные цены округляем вниз)
$awardPoints = max(0, (int)floor((float)$priceEur));

    /* ===== собрать МЕТА из JSON/POST/GET (включая meta_b64 / quiz_*) ===== */
    $metaPayload = null;

    // 1) raw meta / quiz в JSON-теле или в POST
    ...
    // 3) собрать quiz_* из любых источников
    if ($metaPayload === null) {
      $quiz=[];
