...
/* 👉 НОВОЕ: если нет валидных даты/времени — уводим в календарь, сохраняя все параметры квиза */
if (!$okBase) {
  $q = [
    'from'     => 'quiz',
    'lang'     => $lang,
    'svc'      => $svc,
    'services' => $svc, // совместимость со старым кодом
  ];
  if ($slug   !== '') $q['slug']     = $slug;
  ...
  if ($sum_eur > 0) {
    $q['sum_eur']   = (string)$sum_eur;
    $q['price_eur'] = (string)$sum_eur;
  }

  if ($meta_b64 !== '') $q['meta_b64'] = $meta_b64;
  foreach ($quizParams as $k=>$v) { $q[$k] = $v; } // если пришли quiz_*
...
<div class="wrap">
  ...
  <!-- скрытые поля (обновляются автоназначением) -->
...
var SALON = <?= json_encode($salon) ?>; // может быть ''
var STAFF = <?= json_encode($staff) ?>; // может быть ''
...
var url = '/booking/api.php?' + qs.toString();
    ...
    // подписи по ID из URL (если были)
...
async function autoAssignIfAny(){
  ...
  // жёлтая плашка — пояснение если пришли с «Все»
  if (FROM_ANY){
...
// подстраховка: взять из hidden, если есть
if (!payload.meta_b64) {
...
<script>
// === Маска поля телефона (минимальная) ===
(function(){
  ...
  // базовая проверка перед сабмитом (дублируем UX)
