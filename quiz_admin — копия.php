...
function db_sync_services(array $config, array $LANGS){
  ...
  foreach (($config['areas'] ?? []) as $areaKey => $_meta){
    ...
    foreach ($node as $code => $o){
      ...
      $dur = (int)($o['duration_min'] ?? 0);
      $price = (float)($o['price'] ?? 0);
      try{
        $stmt = $pdo->prepare("
          INSERT INTO services (code, title_ru, duration_min, price_eur)
          VALUES (:code,:title,:dur,:price)
        ...
        ");
        $stmt->execute([
          ':code'=>$code, ':title'=>$title_ru, ':dur'=>$dur, ':price'=>$price,
        ]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_all'])) {
    ...
    foreach ($ALLOWED_STEP_KEYS as $stepKey){
      ...
      if (!empty($_POST[$postKey]) && is_array($_POST[$postKey])) {
        foreach ($_POST[$postKey] as $id => $row) {
          ...
          // --- НОВОЕ: slug (только для шага service). Wix booking_url удаляем.
          if ($stepKey==='service') {
            unset($o['booking_url']); // чистим старое поле, если осталось
      ...
      if (!empty($_POST[$addKey])) {
        ...
        if ($name !== '') {
          ...
          if (!isset($new[$areaKey][$stepKey]['options'][$id])) {
            ...
            if ($stepKey==='service') $base += ['slug'=>'']; // вместо booking_url
      ...
      // --- НОВОЕ: автогенерация и уникализация slug для service
  ...
  // ===== optional DB sync (services.duration_min & price_eur) =====
  db_sync_services($config,$LANGS);

  $flash  = 'Изменения сохранены. Для услуг сгенерированы SEO-ссылки (slug). Поле booking_url удалено.';
}
...
function intInput($name,$val,$min='0'){ return '<input type="number" step="1" min="'.h($min).'" name="'.$name.'" value="'.h((string)(is_numeric($val)?(int)$val:0)).'" style="width:110px">'; }
...
<div class="container">
  <h1>Админка квиза</h1>
  <p class="small-note">Перед сохранением создаётся бэкап <code>quiz_config.json</code>. Пустая «Длительность (мин)» = 0. При наличии БД значения синхронизируются в таблицу <code>services</code>. Ссылки (slug) для услуг — для SEO-URL в брони.</p>
      ...
      <div class="area-list">
        <?php foreach ($areas as $key=>$meta): ?>
          <div class="area-item">
            ...
            <div class="small-note">Шаги и контент редактируются ниже.</div>
          </div>
        <?php endforeach; ?>
      </div>
      <hr class="sep">
    ...
    <?php foreach ($areas as $areaKey=>$meta): ?>
      <div class="card" id="sec-steps-<?=$areaKey?>">
        ...
        <table class="steps-table">
          <thead>
            <tr><th>#</th><th>Тип шага</th><th>Вкл</th><th>Порядок</th><th>show_if_service_in</th><th>hide_if_service_in</th></tr>
          </thead>
          <tbody>
      ...
      ?>
        <div class="card" id="sec-<?=$areaKey?>-<?=$stepKey?>">
          ...
          <?php if (!empty($node['options'])): ?>
            <?php foreach ($node['options'] as $id=>$o): ?>
              <div class="opt">
                ...
                <!-- Длительность теперь редактируется НА ВСЕХ ШАГАХ -->
                <div class="row">
                  <label class="small">Длительность (мин):</label>
                  <?=intInput("opt_{$areaKey}_{$stepKey}[{$id}][duration_min]", ($o['duration_min'] ?? 0), 0)?>
                  <span class="small-note">если пусто — 0</span>
                </div>
                ...
                <?php if ($stepKey==='service'): ?>
                  <div class="row">
                    <label class="small">URL-слиг (et):</label>
                    <?=textInput("opt_{$areaKey}_{$stepKey}[{$id}][slug]", $o['slug'] ?? '', 260)?>
                    <span class="small-note">пусто → сгенерируется из названия [et]</span>
                  </div>
                <?php endif; ?>
          ...
          <div class="addrow">
            ...
            <span class="small-note">ID создастся автоматически (на основе названия). Для услуг можно задать свой slug.</span>
          </div>
        </div>
      <?php endforeach; ?>
    <?php endforeach; ?>
